...
// Функция для выполнения алгоритма Прима
void Prims(int graphMatrix[8][8], int start)
{
    ...
    while (currentEdge < 7)
    {
        ...
        // Поиск минимального ребра среди посещённых и не посещённых вершин
        for (int i = 0; i < 8; i++)
        {
            if (visited[i])
            {
        ...
        // Добавляем найденное минимальное ребро в массив остовного дерева
...
// Функция проверки связности графа для алгоритма Краскала
...
int main()
{
    ...
    int P = 1; 
    int mu = E - V + P;
    cout << "\nЦикломатическое число графа: " << mu << endl;

    // Вывод O-большого для каждого алгоритма
    cout << "Временная сложность Прима: O(V^2)" << endl;
    cout << "Временная сложность Краскала: O(E log E)" << endl;
